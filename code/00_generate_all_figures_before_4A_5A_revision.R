...
if (!is.null(alpha_df)) {
	...
	plots_alpha <- Map(function(m, lab) {
		if (!m %in% names(alpha_df)) return(NULL)
		dat <- alpha_df %>% dplyr::filter(!is.na(.data[[m]]), !is.na(group_id))
		if (nrow(dat) == 0) return(NULL)
		ggplot(dat, aes(x = group_id, y = .data[[m]], fill = group_id)) +
			...
			scale_x_discrete(labels = GROUP_LABELS[grp_present]) +
			scale_y_continuous(expand = expansion(mult = c(0.05, 0.10))) +
			labs(title = lab, x = NULL, y = lab) +
			theme_bw(base_size = 10) +
			theme(aspect.ratio = 1, axis.text.x = element_text(angle = 35, hjust = 1),
	...
	p4A <- wrap_plots(plots_alpha, ncol = 3) +
		plot_annotation(
			...
			subtitle = "Rarefied | Observed · Chao1 · ACE · Shannon · Pielou · Dominance",
...
tryCatch({
	...
	df_plot <- df_plot %>%
		...
		x_label = factor(as.character(mouse_id),
						 levels = unique(as.character(mouse_id[order(match(Sample, sample_order))])))
		)
  
	label_map <- df_plot %>%
		dplyr::select(group_id, output_label) %>% dplyr::distinct() %>%
		dplyr::mutate(label_wrapped = stringr::str_wrap(output_label, width = 30))
	grp_labeller <- setNames(label_map$label_wrapped, as.character(label_map$group_id))
	...
	save_fig(p4B, file.path(FIG_DIR, "Fig4B_S2_family_bar"),
					 w = max(10, nsamples(ps_all) * 0.55 + length(present_groups) * 2.2), h = 7)
...
tryCatch({
	...
	aov_5a <- tryCatch(rstatix::anova_test(ds_df, dysbiosis_score ~ group_id),
											error = function(e) NULL)
	aov_p <- if (!is.null(aov_5a) && "p" %in% names(aov_5a)) aov_5a$p[1] else NA_real_
  
	pw_5a <- if (!is.na(aov_p) && aov_p < 0.10) {
		tryCatch(
			rstatix::tukey_hsd(ds_df, dysbiosis_score ~ group_id) %>%
				dplyr::mutate(p.adj = p.adjust(p.adj, method = "BH")) %>%
				rstatix::add_significance("p.adj", cutpoints = c(0, 0.001, 0.01, 0.05, 1),
																	symbols = c("***", "**", "*", "ns")),
			error = function(e) NULL)
	} else NULL
  
	sig_5a <- if (!is.null(pw_5a)) {
		sig <- pw_5a %>% dplyr::filter(p.adj.signif != "ns") %>%
			dplyr::arrange(group1, group2)
		if (nrow(sig) > 0) {
			y_max <- max(ds_df$dysbiosis_score, na.rm = TRUE)
			y_step <- diff(range(ds_df$dysbiosis_score, na.rm = TRUE)) * 0.12
			dplyr::mutate(sig, y.position = y_max + y_step * seq_len(nrow(sig)))
		} else NULL
	} else NULL
  
	if (!is.null(pw_5a))
		write_csv(pw_5a, file.path(TBL_DIR, "Fig5A_dysbiosis_score_stats.csv"))
  
	p5A <- ggplot(ds_df, aes(x = group_id, y = dysbiosis_score, fill = group_id)) +
		geom_boxplot(alpha = 0.7, outlier.shape = NA, width = 0.5) +
		geom_jitter(width = 0.13, size = 2.2) +
		scale_fill_manual(values = GROUP_PALETTE[intersect(names(GROUP_PALETTE),
																												levels(ds_df$group_id))],
											guide = "none") +
		scale_x_discrete(labels = GROUP_LABELS[levels(ds_df$group_id)]) +
		...
		labs(title = "Figure 5A — Bray-Curtis dysbiosis score (S2)",
				 subtitle = paste0("BC distance to ", REF_GROUP,
													 if (!is.na(aov_p)) sprintf(" | ANOVA p = %.3f", aov_p) else ""),
...
tryCatch({
	...
	outcomes_5b <- c("wt_chg_pct", "fat_pct", "lean_pct")
	...
	plots_5b <- lapply(outcomes_5b, function(oc) {
		...
		ggplot(df_5b, aes(x = dysbiosis_score, y = .data[[oc]], colour = group_id)) +
			...
			geom_point(size = 3) +
			scale_colour_manual(values = GROUP_PALETTE[intersect(names(GROUP_PALETTE),
																														unique(df_5b$group_id))]) +
			annotate("text", x = Inf, y = Inf, hjust = 1.1, vjust = 1.5,
							 label = annot, size = 3) +
			labs(title = paste0(oc, " ~ Dysbiosis score"),
					 x = "Dysbiosis score", y = oc) +
			theme_bw(base_size = 10) +
			theme(aspect.ratio = 1,
						legend.position = if (oc == "lean_pct") "right" else "none")
	})
	plots_5b <- Filter(Negate(is.null), plots_5b)
  
	if (length(plots_5b) > 0) {
		p5B <- wrap_plots(plots_5b, nrow = 1) +
			plot_annotation(
	...
	if (length(scfa_avail) > 0) {
		...
		p5D <- ggplot(guild_long_5d, aes(x = sample_id, y = RA, fill = guild)) +
			...
			facet_grid(. ~ group_id, scales = "free_x", space = "free_x",
								 labeller = labeller(group_id = GROUP_LABELS)) +
		...
		save_fig(p5D, file.path(FIG_DIR, "Fig5D_scfa_guild_stacked_bar"), w = 10, h = 5)
	}
